...
3. Check `OWNERS.md` for who to contact about tokens or code.  
...
- **Canonical token file:** `tokens/tokens.json` (edit here only; see tokens README).  
...
*(Replace `@name` with team handles; update `OWNERS.md` as the authoritative file.)*
...
- **Adoption:** 25% of new UI code uses `@org/ui` components (measured by `metrics/token-usage-scan.js` or manual repo checks).  
...
> We will improve these after baseline collection — see `docs/measurements/measurement-plan-template.md`.
...
- Roadmap: `ROADMAP.md` (this repo)  
- Measurement plan: `docs/measurements/measurement-plan-template.md`
...
- A developer: follow the "Canonical token file" rule — tokens live at `tokens/tokens.json`.

---

## Change log
- _YYYY-MM-DD_ — Created initial project README.  
- (Use `CHANGELOG.md` for more granular release history.)